...
if __name__ == '__main__':
    ...
    grades.sort(key=lambda k: k[1])  # sort by grade
    grades = list(filter(lambda x: x[1] != grades[0][1], grades))  # remove lowest scores
    grades = list(filter(lambda x: x[1] == grades[0][1], grades))  # filter by score
    grades.sort(key=lambda k: k[0])  # sort by name

    print('\n'.join([x[0] for x in grades]))
